        ...
        <>
            {(cnsForThisUser.length > 0 || authContext.user?.isCreator == true) && (showTable) ? (
                <div className="scrollableContainer" ref={scrollableContainerRef} >
                    ...
                    <div className="iconContainer ent-requires-selection" style={{ opacity: iconDisplayState[14] }} onClick={(e) => navigate('/')}><i className={`material-icons ent-icon ent-blue`}>bar_chart</i><div>Reports</div></div>
                </div>
            ) : <></>}
            {showTable && showTable &&
                <hr></hr>}
